...
namespace Serial_Comm_Tester  {
    public partial class COMForm : Form {  // public Conversion_Table conTable = null;
        ...
        private void SerialEncoding()   //comm 解码
        {
        ...
        void getAvailablePorts( )     //this code just finds the available ports to use
        {            //this code gets the name of the port and port number
            ...
            comboBoxActiveComPorts.Items.AddRange(ports);  //one seed Com group to combaox
        }
        private void Form1_Load(object sender, EventArgs e)          {
            comboBoxDecodeFormat.SelectedIndex = 0;            comboBoxParity.SelectedIndex = 0;
            comboBoxDataBits.SelectedIndex = 3;            comboBoxStopBits.SelectedIndex = 0;
           comboBoxFlow.SelectedIndex = 0; graph_speed.Value = 10;  //set time  10ms
        ...
        public void btnOpenPort_Click(object sender, EventArgs e)  {   //打开串行口
            try
            {
           ...
           //checkBoxSendBinary.Enabled = true;    //checkBoxSendOct.Enabled = true;
                    }
                    ...
                    //{    //  textBoxRTS.BackColor = Color.Lime;    //    textBoxRTS.Text = "Rts ON";   //}       //else
                    //{    //    textBoxRTS.BackColor = Color.Red;  //   textBoxRTS.Text = "Rts Off";  //}
         ...
         //   chkBAutoReadHex.Enabled = true; chkBAutoReadHex.Checked = true;
        }
        ...
        private async void btnClosePort_Click(object sender, EventArgs e)   {  //Close COM
       ...
       //         chkBAutoReadHex.Enabled = false;
        ...
        //serialPort1.DiscardInBuffer();    // yd 源程序 这四行 是关闭的， 是不是 有关
   ...
   //         chkBAutoReadHex.Enabled = false;
        ...
        private  void btnSend_Click(object sender, EventArgs e)   //发送COM SHJ  >>>>>>>bool decsend
        {
        ...
        private void displayAutoHex(object o, EventArgs e)  //线程调用读com 数据
        {
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)    {  //？？关闭
            //if (serialPort1.IsOpen)         {
            //    try          {
            //        serialPort1.Close();
            //        serialPort1.DiscardInBuffer();
        ...
        private async void btnSendFile_Click(object sender, EventArgs e)
        {
            if (serialPort1.IsOpen)
            {
                ...
                ofd.Filter = "text files (*.txt)|*.txt|Richtext files (*.rtf)|*.rtf |Any file(*.*)| *.*";
        ...
        private void GetChartPoint(int input ) {   //This adds points to the chart graph
            lblCountRX.Update();
        ...
        private void ckBStartGraph_CheckedChanged(object sender, EventArgs e) {  //接收数据图像显示
        ...
        private void tabControl2_Click(object sender, EventArgs e) {    //yd  2019-6-5
            ckBStartGraph.Checked = false; chkBAutoReadHex.Checked = false;
            chkBAutoRead.Checked = false; chkBAutoReadDec.Checked = false;
            int i = tabControl2.SelectedIndex;
            //if (tabControl1.SelectedTab.Text == "tabHexRead") {
            if (i == 1) {
                chkBAutoReadHex.Enabled = true;
                chkBAutoReadHex.Checked = true;  //tabpage2set = false; //next no select
           }
             if (i == 0 ) {
                chkBAutoRead.Checked = true;   
            }
            if (i == 3) {   //图像显示
                ckBStartGraph.Checked = true;       ckBStartGraph.Enabled = true; 
            }
            if (i == 2) {   //DEC
                chkBAutoReadDec.Checked = true; chkBAutoReadDec.Enabled = true;
            }
        }
